...
Sys.setenv(TZ="UTC")
scriptname <- "s6_swarmCalcContext.R" # for logging
...
while(TTL > 0) {
  ...
  this.line <- getOldestOpenDate(cono)
  if(nrow(this.line) == 1) {
    ...
    setProcessStartMark(cono, this.date, hname)
    # verify if startmark is set
    vf <- verifyProcessMark(cono, this.date, hname)
    if(nrow(vf) == 1) {
      ...
      source("/home/voellenk/osorno_workdir/osorno/lib/calc_Context.R")
      
      # set finishmark
      setProcessStopMark(cono, this.date, nsyms)
